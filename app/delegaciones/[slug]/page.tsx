...
export default async function DelegacionPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const path = `/delegaciones/${slug}`;
      ...
      {!hasBlocks && (
        <>
          <div className="space-y-2">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Delegaciones</p>
